...
owid <- read.table('../data/stage3/epi_owid.csv',header=1, sep=',')


# Introduction_file='/home/fra/Projects/EmergingVariants/Omicron/Results/Introductions/Alpha/Alpha'
# Parameters_file = '/home/fra/Projects/EmergingVariants/Omicron/Params/Renewal_aug_params/Parameters_Alpha.csv'
# output_path = '/home/fra/Projects/EmergingVariants/Omicron/Results/Projections/Renewal_aug/Alpha'
# first_line = 1
# last_line = 0
# output_prefix = 'Projections'
...
for (line in seq(first_line,last_line)){
  ...
  S0 <- params$pop[line]  #Population in target country
  ...
  target_country<-owid[owid$iso_code==target,]
  condition_start= as.Date(target_country$date) >= as.Date(date_start)-7
  condition_end = as.Date(target_country$date) < as.Date(date_end)
  target_country <- target_country[condition_start & condition_end,]
  R_t <- target_country$reproduction_rate*R/target_country$reproduction_rate[1]
  ...
  if (N<20){
    N=20
    date_end=as.Date(date_start)+20
  }  
  
  if (N==0){
    print(paste("This country:",target,"has date_end = date_start, skipping"))
    next
  }    
  variant <- params$variant[line] 
  introductions <- paste(Introduction_file,source,target,date_start, date_end,multiplier_in,multiplier_out, sep = "_")
  skip_to_next <- FALSE
  ...
  multiplier_out <- as.numeric(multiplier_out)
  multiplier_in <- as.numeric(multiplier_in)
...
Renewal_process <- function(N,I_0,R_t){
  ...
  for (day in seq(from = 2, to =  N)) {
    ...
    #This comes from Epidemia: R_tilde is the true time-varying reproduction number
    #R_t is the basic reproduction number: https://imperialcollegelondon.github.io/epidemia/articles/introduction.html
    R_tilde <- R_t[[day]]
    incidence_today <- R_tilde * sum(contributing_incidences * weights)
...
print("check codes/warnings_log.RData for warnings")
save(warning_messages, file="warnings_log.RData")
